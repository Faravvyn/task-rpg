...
import { Check, Trash2, Edit3, ChevronDown, ChevronUp, RotateCcw, Clock, Star } from 'lucide-react'
...
export default function TaskItem({ task, completions, onEdit }) {
  ...
  const handleComplete = async (e) => {
    if (!available || completing) return
  ...
  return (
    <div className={`card relative overflow-hidden transition-all duration-300 ${!available?'opacity-60':'hover:border-gold-700/20'} ${completedToday?'border-green-800/30':''}`}>
      ...
      <div className="flex items-center gap-3">
        <button onClick={handleComplete} disabled={!available||completing}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs text-gold-500 font-semibold">+{baseXp} XP{streak>0&&<span className="text-gold-300 ml-1">(🔥{Math.round((getStreakMultiplier(streak)-1)*100)}%)</span>}</span>
          </div>
        </div>
