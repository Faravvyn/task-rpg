...
export default function Layout() {
  ...
  const { pendingFriendTasksCount } = adventure
  ...
  return (
    <div className="min-h-screen bg-dark-500 flex flex-col lg:flex-row">
      {xpPopup && (
        <div 
          className="fixed pointer-events-none z-[100] animate-float flex flex-col items-center"
          style={{ left: xpPopup.x, top: xpPopup.y - 40, transform: 'translateX(-50%)' }}
        >
          {xpPopup.isCrit && <span className="text-gold-400 font-black text-xl neon-glow animate-bounce">KRITISCH!</span>}
          {xpPopup.isFrenzy && <span className="text-orange-500 font-bold text-[10px] uppercase tracking-widest">Frenzy Bonus</span>}
          <span className={`font-black text-2xl ${xpPopup.isCrit ? 'text-gold-300 scale-125' : 'text-gold-400'}`}>
            +{xpPopup.xp} XP
          </span>
          <span className="text-gold-500 font-bold text-sm flex items-center gap-1">
             💰 +{xpPopup.gold} Gold
          </span>
        </div>
      )}
