...
export default function TasksPage() {
  ...
  const [form,setForm]=useState({title:'',category:'haushalt',customCategory:'',difficulty:'mittel',repeat_type:'taeglich'})
  const resetForm=()=>{setForm({title:'',category:'haushalt',customCategory:'',difficulty:'mittel',repeat_type:'taeglich'});setShowForm(false);setEditingTask(null);setFormError('')}
  const handleSubmit=async(e)=>{
    ...
    if(editingTask){const{error}=await editTask(editingTask.id,{title:form.title.trim(),category,difficulty:form.difficulty,repeat_type:form.repeat_type});if(error){setFormError('Fehler: '+(error.message||'Unbekannt'));return}}
    else{const{error}=await createTask({title:form.title.trim(),category,difficulty:form.difficulty,repeat_type:form.repeat_type});if(error){setFormError('Fehler: '+(error.message||'Unbekannt'));return}}
    resetForm()
  }
  const startEdit=(task)=>{setEditingTask(task);setForm({title:task.title,category:task.category,customCategory:'',difficulty:task.difficulty,repeat_type:task.repeat_type});setShowForm(true)}
  const filteredTasks=(()=>{
