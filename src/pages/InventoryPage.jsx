...
import { RARITIES, rarityInfo } from '../utils/adventure'

export default function InventoryPage() {
  const { characterArtifacts, equipArtifact, toggleProtect, grantRandomArtifact, equippedItems, protectedCount } = useAdventure()
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {filtered.length === 0 ? (
        <EmptyState emoji="💎" title="Leere Schatztruhe" subtitle="Noch keine Artefakte. Besiege Bosse, erfülle Quests oder gewinne Duelle!"
          action={<button onClick={() => grantRandomArtifact('common')} className="btn-secondary inline-flex items-center gap-2 text-sm"><Sparkles className="w-4 h-4" /> Test-Artefakt finden</button>} />
      ) : (
        <>
          ...
          <button onClick={() => grantRandomArtifact('common')} className="btn-secondary w-full inline-flex items-center justify-center gap-2 text-sm">
            <Sparkles className="w-4 h-4" /> Test-Artefakt finden (Demo)
          </button>
        </>
      )}
    </div>
